...
function State(name, ind) constructor{
	state_name = name;
	state_index = ind;
	linear = true;
	frames = [];
	...
	if linear {
		...
		switch state_name {
			case "idle": first_frame =[0,8,16,24]; length = 0; break;
			case "walk": first_frame =[32,40,48,56]; length = 6; break;
			case "push": first_frame =[1,9,17,25]; length = 2; break;
			case "pull": first_frame =[3,11,19,27]; length = 2; break;
		}
		...
		switch state_name {
			case "jump": frames=[[5,6,6,7,7],[13,14,14,15,15],[21,22,22,23,23],[29,30,30,31,31]]; length=5; break;
			case "run": frames=[[32,33,38,34,35,39],[40,41,46,43,44,47],[48,49,54,51,52,55],[56,57,62,59,60,63]]; length = 6; break; 
		}
